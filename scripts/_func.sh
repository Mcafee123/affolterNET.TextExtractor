...
not_empty() {
    if [ -z $1 ]; then
        exit_script "variable '$2' must not be empty"
    fi
}
...
source_file() {
    ...
    if test -f "$source_file"; then
        ...
        exit_script "FILE DID NOT EXIST $source_file"
    fi    
}
...
delete_serviceprincipal() {
    get_serviceprincipal
    spId=$( echo $service_principal | jq -r .[0].id )
...
create_serviceprincipal() {
    ...
    if [ "$service_principal" == "[]" ]; then
        ...
        spId=$( echo $service_principal | jq -r .[0].id )
        result=$(az ad sp credential reset --id "$spId")
        client_secret=$( echo $result | jq -r .password )
    fi

    appId=$( echo $service_principal | jq -r .[0].appId )
    echo "app id: $appId"
    tenant_id=$( echo $service_principal | jq -r .[0].appOwnerOrganizationId )
    echo "tenant id: $tenant_id"
    displayName=$( echo $service_principal | jq -r .[0].appDisplayName )
    ...
    echo
    echo "create __${env}_sp.sh"
    echo "export SP_DEVOPS_ID=\"$appId\"" > __${env}_sp.sh
    echo "export SP_TENANT_ID=\"$tenant_id\"" >> __${env}_sp.sh
    echo "export SP_DEVOPS_PASSWORD"=\"$client_secret\" >> __${env}_sp.sh
    echo
}
...
create_share() {
  echo "create share:"
  app_name=$1
  not_empty "$app_name" "app_name"
  storageaccount_name=$2
  not_empty "$storageaccount_name" "storageaccount_name"
  storageaccount_key=$3
  not_empty "$storageaccount_key" "storageaccount_key"
  custom_id=$4
  not_empty "$custom_id" "custom_id"
  share_name=$5
  not_empty "$share_name" "share_name"
  mount_path=${6:-"/"}
  storage_type=${7:-"AzureFiles"}

  echo
  echo "app_name: $app_name"
  echo "storageaccount_name: $storageaccount_name"
  echo "storageaccount_key: `echo $storageaccount_key | cut -c1-3`***"
  ...
  echo
  # exit 0
  # mounts=$(az webapp config storage-account list --resource-group "$rg" --name "$app_name" --query "[?name == '$custom_id']")
  # exit 0

  mounts="[]"

  if [ "$mounts" == "[]" ]; then
    echo "create share $custom_id"
    az webapp config storage-account add \
      ...
      --access-key "$storageaccount_key" \
      --mount-path "$mount_path"
    exit 0
    createResult=$(az webapp config storage-account add \
      --resource-group "$rg" \
      --name "$app_name" \
      --custom-id "$custom_id" \
      --storage-type "$storage_type" \
    ...
    echo "update share $custom_id"
    updateResult=$(az webapp config storage-account update \
      --resource-group "$rg" \
      --name "$app_name" \
      --custom-id "$custom_id" \
      --storage-type "$storage_type" \
